...
function ItemDAO(database) {
  ...
  this.getCategories = function(callback) {
     ...
     * this.db.item.aggregate([ { $group: { _id: "$category" , num: { $sum: 1 } } }, { $sort : { _id : 1 } } ]).toArray()
     *
     */
    var categories = [];
    var k = 0;
  ...
  this.getItems = function(category, page, itemsPerPage, callback) {
     ...
     */
    var pageItems = [];
     var pageskip = page*itemsPerPage;
  ...
  this.db.collection('item')
     ...
     * getNumItems() method.
     * db.item.find({"category" :"Apparel" }).count()
     */
  ...
  this.searchItems = function(query, page, itemsPerPage, callback) {
     ...
     *  db.item.find( { $text : { $search : query } } )
     */
         var items = [];
         var quer =  { $text : { $search : query } };
         var pageskip = page*itemsPerPage;
         this.db.collection('item')
         .createIndex( { title: "text",slogan: "text",description: "text" } );

             this.db.collection('item').find(quer)
              .skip(pageskip)
             .limit(itemsPerPage)
