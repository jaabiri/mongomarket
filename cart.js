...
function CartDAO(database) {
    ...
    this.getCart = function(userId, callback) {
        ...
        * callback function.
        *db.cart.find({"userId" : '558098a65133816958968d88' }).toArray(function(item){
        * item.items.forEach(function(elem){ console.log(elem.title)})
        *})
        */
        ...
        var userCart = {
          ...
          var query = {"userId" :userId };
            var cursor = this.db.collection('cart')
            .find(query)
            .toArray(function(err, result) {
              console.log();
       result[0].items.forEach(elem => {
               userCart.items.push(elem);
            });
          callback(userCart);
          });
